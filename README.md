...
| Doc comments (`go doc`) | **Verified 2026-09-20, mechanically** | Every top-level `func`, `type`, `const`, and `var` in the repository carries a doc comment — **226 of 226**, exported and unexported, test helpers included — checked by script rather than by eye, and re-checked after `gofmt` confirmed the files were still clean. See `CLAUDE.md` for the audit command and the gofmt alignment trap that adding these comments can spring. |
