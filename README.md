...
| `internal/mrf` (JSON + CSV parsing, MRF download) | **Unit-tested, real** | `go test ./internal/mrf/...` — 15 tests (46 including table subtests), all passing, against hand-built JSON and CSV (tall + wide) fixtures covering the CMS field set. Eleven of those were added on 2026-09-25 with the SSRF fix below: the address classifier across 22 ranges, the dial-time `Control` hook, the redirect cap and scheme policy, the size cap, and end-to-end assertions that a loopback URL is refused *without the server's handler ever running* — the difference between reporting an SSRF and preventing one. |
...
| `internal/store`, `internal/auth`, `internal/config`, `internal/validation`, `internal/web` | **Unit- and integration-tested 2026-09-23** | These five packages had no `_test.go` files until now; they have 52 tests between them, all passing. `internal/auth` covers token entropy and shape, the raw-never-equals-hash property, and every session-cookie attribute. `internal/config` covers defaults, overrides, the four unit conversions, and the fail-fast-on-a-typo behavior. `internal/store` runs against a real Postgres and includes the three ownership-scoping invariants (`GetHospital`, `ListHospitalsByOwner`, `GetRunReport` all deny another user), single-use magic links, and expiry enforcement. `internal/validation` drives the whole pipeline end to end against an `httptest` MRF server and asserts a run always reaches a terminal status — the regression guard for the "stuck on Checking… forever" bug. `internal/web` pins the `derefBool` bug below by rendering the report page and reading the banner, plus the auth gate on all six protected routes. The database-backed suites **skip** when `DATABASE_URL` is unset and **fail** when it is set but unreachable, so a broken CI database cannot silently retire them. `cmd/server` is still untested — it is wiring only. Writing these also turned up a security defect that no test had previously covered and that running the app would never have shown: `CookieShouldBeSecure` compared `PUBLIC_BASE_URL` against the literal `"https://"`, so the legal spelling `HTTPS://…` (URL schemes are case-insensitive per RFC 3986), or a value carrying a stray leading space, was read as plain HTTP and silently produced a session cookie with no `Secure` flag. Fixed on 2026-09-24 by lowercasing and trimming first, with the cookie itself asserted end to end rather than only the predicate. Noted here rather than in the bug tally above because it was found by reading the code while writing tests, not by executing the app. `internal/validation` gained three more tests on 2026-09-25 alongside the SSRF fix, including one that asserts a refused connection and a blocked address produce byte-identical text on the report page — different messages there would turn the page into an internal port scanner. |
| Four real defects found this way | **Found and fixed** | (1) `pq.Array(nil)` for a rule with zero sample failures serialized to SQL `NULL`, violating a `NOT NULL` column constraint — only appeared when a real save happened against a real schema. Fixed in `internal/store/runs.go`. (2) `html/template`'s `{{if}}` on a `*bool` only checks non-nil, never dereferences — the dashboard showed "Compliant" for runs that had actually failed. Fixed via a `derefBool` template function (`internal/web/templates.go`). Both fixes were re-verified by re-running the same end-to-end flow and confirming correct output. (3) `Migrate` was not safe to run concurrently: `CREATE TABLE IF NOT EXISTS` is not atomic against a concurrent creator in Postgres, so two processes migrating the same *fresh* database both saw the table missing, both tried to create it, and the loser failed with `duplicate key value violates unique constraint pg_type_typname_nsp_index`. Surfaced on 2026-09-23 by the new test suite — `go test ./...` runs package binaries concurrently, and both `internal/store` and `internal/validation` migrate on the way in. It reached CI rather than being caught locally, because a developer database already carrying the schema is exactly the condition that hides it. This was a production bug too, not a test artifact: `cmd/server` migrates on every startup, so any deploy starting two tasks together took the same path — relevant the moment the ECS service in `DEPLOY.md` scales past one task. Fixed with a Postgres advisory lock held on a dedicated connection (`internal/store/migrate.go`), with a test that runs four concurrent migrations. (4) **Server-side request forgery in the MRF downloader** (CWE-918), found by a security review of the branch on 2026-09-25 and fixed the same day. The MRF URL is user-supplied and was passed to `http.DefaultClient` with no destination check beyond a `strings.HasPrefix` scheme test in the dashboard handler, so any signed-up user could point a hospital at `http://169.254.169.254/latest/meta-data/…` and read the result off their own report page. Fixed in `internal/mrf/fetch.go` with a `net.Dialer.Control` hook that refuses non-public addresses at dial time — after DNS resolution, before `connect`, once per connection, which is what makes it hold against DNS rebinding and across redirect hops that a submit-time URL check cannot see. The second half of the fix is that the stored error message no longer carries the transport error: a refused connection, an unresolvable host, a timeout and a blocked address now all read the same on the page, with the detail kept in the structured log. This one was pre-existing rather than introduced by recent work, and it is the only defect here that no amount of running the app would have surfaced — it needed someone to read the code and ask what the URL was trusted to be. See `ARCHITECTURE.md` for the second and fourth ones' mechanisms in detail — both are general Go gotchas worth understanding, not one-off fixes. |
...
| Doc comments (`go doc`) | **Verified 2026-09-20, mechanically** | Every top-level `func`, `type`, `const`, and `var` in the repository carries a doc comment — **229 of 229**, exported and unexported, test helpers included — checked by script rather than by eye, and re-checked after `gofmt` confirmed the files were still clean. See `CLAUDE.md` for the audit command and the gofmt alignment trap that adding these comments can spring. |
| Documentation set | **Complete as of 2026-09-20** | `README.md`, `SRS.md`, `ARCHITECTURE.md`, `UI-DESIGN.md`, `AUTH.md`, `DEPLOY.md`, all cross-linked from the map at the top of this file. The contrast ratios quoted in `UI-DESIGN.md` were computed from the actual CSS tokens, not estimated. One pair originally failed WCAG AA — the pending badge at 4.18:1 — and the `--pending` token was darkened to `#855c00` (5.31:1) to fix it; every pair in the table now passes. §5.4 listed six further accessibility gaps; five are now closed (skip link, `aria-live` on the polling banner, `scope="col"` on every `<th>`, a `title` on the truncated MRF URL, and a `:focus-visible` treatment), each with a test in `internal/web` to keep it from regressing. The sixth stays open and is the one that would validate the rest: nothing here has been tested with an actual screen reader. |
...
One of them is a security control rather than a convenience. `ALLOW_PRIVATE_MRF_ADDRESSES` defaults
to `false`, and while it is false the MRF downloader refuses to connect to any address that is not on
the public internet — loopback, RFC 1918, link-local (which is how the cloud metadata endpoint at
`169.254.169.254` is caught), and the other reserved ranges. Setting it to `true` turns that off, and
it exists for the case where you are pointing a local checkout at a file server on `localhost`. Do
not set it anywhere other people can add a hospital: the MRF URL is theirs to choose, and with the
filter off this server will fetch whatever it can reach on their behalf. See `ARCHITECTURE.md`'s
